...
If we allow for bulges due to differing growth rates and assume that the colony is not smooth, we must account for stochastic undulations that build up at the front. We use a lattice-based algorithm, the [Eden Model](https://en.wikipedia.org/wiki/Eden_growth_model), to explore this problem. A short [IPython notebook](https://github.com/Range-Expansions/rough_front_expansion/blob/master/doc/radial_expansion_D2Q9.ipynb) can be seen illustrating how to use the package.
